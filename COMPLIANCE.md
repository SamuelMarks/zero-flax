...
The `ml-framework-snapshots` tool verifies that `zero-flax` accurately replicates the public API signatures, parameters, and defaults of the real `flax` framework.

**To run the checker against the current codebase:**

```bash
ml_framework_snapshots check flax ~/repos/zero-flax/src \
    --reference-prefix flax \
    --target-prefix zero_flax
```

**What it does:**
- It statically analyzes all Python modules in `zero-flax/src`.
- It maps the target namespace (`zero_flax.*`) back to the reference framework (`flax.*`).
...
```bash
ml-switcheroo-ir compliance ~/repos/zero-flax/src
```
